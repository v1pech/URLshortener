...
func (s *Storage) SaveURL(url, alias string) (int8, error) {
	...
	var exists = false
	_ = s.Database.QueryRow("SELECT EXISTS(SELECT 1 FROM urls WHERE alias = $1)", alias).Scan(&exists)
	if exists {
		return ErrAliasExists, fmt.Errorf("%s: %s", op, "alias already exists")
	}
	_, err := s.Database.Exec("INSERT INTO urls (alias, url) VALUES ($1, $2)", alias, url)
	if err != nil {
